"""Agent implementations.

Each agent is a callable: (state: ForgeState) -> ForgeState. Agents must not
mutate the state dict in-place beyond appending to lists or setting their own
output keys — return the updated state.
"""

from forge.agents.analysis import analysis_agent
from forge.agents.patch import patch_agent
from forge.agents.recon import recon_agent
from forge.agents.validation import validation_agent

__all__ = [
    "recon_agent",
    "analysis_agent",
    "patch_agent",
    "validation_agent",
]
